...
import { Search, ShoppingCart, Phone, Menu, X, Truck, Shield, RotateCcw, Sparkles } from "lucide-react";
    ...
    <>
      ...
      <div className="bg-dark-900 text-white/90 text-[11px] sm:text-xs py-1.5 hidden sm:block">
        <div className="max-w-7xl mx-auto px-4 flex items-center justify-center gap-6 sm:gap-10">
          ...
          <span className="flex items-center gap-1"><Shield size={11} /> Warranty</span>
          <span className="flex items-center gap-1"><RotateCcw size={11} /> Easy Returns</span>
        </div>
      </div>
        ...
        <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-14 sm:h-16">
            ...
            <Link href="/" className="flex items-center gap-2 flex-shrink-0">
              <div className="w-7 h-7 sm:w-8 sm:h-8 bg-dark-900 rounded-lg flex items-center justify-center">
            ...
            <div className="hidden md:flex items-center gap-1">
              {navLinks.map((link) => (
                <Link key={link.name} href={link.href} className="px-3 py-2 text-sm font-medium text-dark-600 hover:text-dark-900 hover:bg-dark-50 rounded-lg transition-all">
                  {link.name}
                </Link>
              ))}
            </div>
